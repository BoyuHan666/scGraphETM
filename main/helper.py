...
def get_theta_GNN(model, RNA_tensor_normalized, ATAC_tensor_normalized):
    model.eval()
    with torch.no_grad():
...
def train_GNNscETM(model, optimizer, RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized, adata,
    ...
    for i in range(niter):
        ...
        perf[i, 1] = NELBO
        if (i % ari_freq == 0):
            theta, theta_gene, theta_peak = get_theta_GNN(model, RNA_tensor_normalized, ATAC_tensor_normalized)
