...
class H5AD_Dataset(torch.utils.data.Dataset):
    def __init__(self, rna_path, atac_path, num_of_cell=2000, num_of_gene=2000, num_of_peak=2000):
        ...
        self.gene_highly_variable_index = get_highly_variable_index(self.rna)

        self.rna = self.rna[:num_of_cell, self.gene_highly_variable_index]
        ...
        self.peak_highly_variable_index = get_highly_variable_index(self.atac)
        #
        self.atac = self.atac[:self.num_of_cell, self.peak_highly_variable_index]
        self.atac_normalized = atac_normalized[:self.num_of_cell, self.peak_highly_variable_index]

        #compute overall correlation graph
        # self.cor_matrix = correlation.get_two_modality_cor(
        #     scRNA_adata=self.rna, scATAC_adata=self.atac)
        self.trainSet = []
    ...
    def getBatch(self, batchSize):
        ...
        indices = random.sample(range(self.num_of_cell), batchSize)
        rna_subsample = self.rna[indices, :self.num_of_gene].copy()
        rna_subsample_norm = self.rna_normalized[indices, :self.num_of_gene].copy()

        atac_subsample = self.atac[indices, :self.num_of_peak].copy()
        atac_subsample_norm = self.atac_normalized[indices, :self.num_of_peak].copy()

        return rna_subsample, atac_subsample

    def getTrain(self):
        rna_subsample = self.rna[:self.num_of_cell, :2000].copy()
        rna_subsample_norm = self.rna_normalized[:self.num_of_cell, :2000].copy()

        atac_subsample = self.atac[:self.num_of_cell, :2000].copy()
        atac_subsample_norm = self.atac_normalized[:self.num_of_cell, :2000].copy()

        print("=======  generate peak-gene cor_matrix  ======")
        cor_matrix = correlation.get_two_modality_cor(
            scRNA_adata=rna_subsample, scATAC_adata=atac_subsample)

        return rna_subsample, rna_subsample_norm, atac_subsample, atac_subsample_norm, cor_matrix

    def getTest(self):
        rna_subsample = self.rna[:self.num_of_cell, :self.num_of_gene].copy()
        rna_subsample_norm = self.rna_normalized[:self.num_of_cell, :self.num_of_gene].copy()

        atac_subsample = self.atac[:self.num_of_cell, :self.num_of_peak].copy()
        atac_subsample_norm = self.atac_normalized[:self.num_of_cell, :self.num_of_peak].copy()
        return rna_subsample, rna_subsample_norm, atac_subsample, atac_subsample_norm

    def getTrainSet(self, batchSize):
        ...
        for i in range(50):
            ...
            self.trainSet.append((rna_subsample, atac_subsample))
