...
def train_GCNscETM_ELBO(model, optimizer, RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized):
...
def train_GCNscETM(model, optimizer, h5ad_dataloader, device, ari_freq, niter=100):
    ...
    best_ari = 0
    best_theta = None
    data_iter = iter(h5ad_dataloader)
    for i in range(niter):
        # for i, batch in enumerate(h5ad_dataloader):
        # Get a random batch
        batch = next(data_iter)
        scRNA_adata, scRNA_adata_normalized, scATAC_adata, scATAC_adata_normalized, cor_matrix = batch
        print(scRNA_adata_normalized)
        print(scATAC_adata_normalized)
        print("======  convert to gpu tensor  ======")
        RNA_tensor = june13_dataloader.to_gpu_tensor(adata=scRNA_adata, device=device)
        RNA_normalized_tensor = june13_dataloader.to_gpu_tensor(adata=scRNA_adata_normalized, device=device)
        # normalize by sum(1)
        ATAC_tensor = june13_dataloader.to_gpu_tensor(adata=scATAC_adata, device=device)
        ATAC_normalized_tensor = june13_dataloader.to_gpu_tensor(adata=scATAC_adata_normalized, device=device)

        edge_index = correlation.convert_to_edge_index(cor_matrix=cor_matrix, device=device)
        model.set_edge_index(edge_index)


        NELBO = train_GCNscETM_ELBO(model, optimizer, RNA_tensor, RNA_normalized_tensor, ATAC_tensor,
                                  ATAC_normalized_tensor)
        theta, theta_gene, theta_peak = get_theta_GCN(model, RNA_normalized_tensor, ATAC_normalized_tensor)
        perf[i, 0] = i
        perf[i, 1] = NELBO
        if (i % 1 == 0):
            ari = evaluate_ari(theta.to('cpu'), scRNA_adata)
            ari1 = evaluate_ari(theta_gene.to('cpu'), scRNA_adata)
            ari2 = evaluate_ari(theta_peak.to('cpu'), scATAC_adata)
            idx = (int)(i / ari_freq)
            ari_perf[idx, 0] = idx
            ari_perf[idx, 1] = ari
            ari_perf[idx, 2] = ari1
