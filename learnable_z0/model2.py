...
class GNN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_heads, device, dropout, conv_model):
        ...
        # self.initial_embedding = nn.Embedding(16131, in_channels)
        # self.initial_embedding.weight.data.normal_(0, 1)
        self.initial_embedding = torch.randn((16131, in_channels), requires_grad=True).to('cuda:0')
