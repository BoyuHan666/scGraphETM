...
def train_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, graph_dec, decoder1, decoder2, optimizer,
    ...
    kl_theta1 = torch.tensor(rna_kl).to('cuda:0')
    kl_theta2 = torch.tensor(atac_kl).to('cuda:0')

    # nll1 = F.binary_cross_entropy(pred_RNA_tensor, RNA_tensor_normalized)
    # nll2 = F.binary_cross_entropy(pred_ATAC_tensor, ATAC_tensor_normalized)
    # print(f'nll1: {nll1.item()}, nll2:{nll2.item()}, sum: {(nll1+nll2).item()}')
    # recon_loss1 = -(pred_RNA_tensor * RNA_tensor_normalized).sum(-1)
    # recon_loss2 = -(pred_ATAC_tensor * ATAC_tensor_normalized).sum(-1)
    # recon_loss = (recon_loss1 + recon_loss2)
    # kl_loss = (kl_theta1 + kl_theta2)
    # # print(pred_RNA_tensor.shape)
    # # print(RNA_tensor_normalized.shape)
    ...
    recon_loss = (recon_loss1 + recon_loss2)
    kl_loss = (kl_theta1 + kl_theta2)
    # print(kl_theta1.shape)
    # print(recon_loss2.shape)
    beta = current_epoch / epochs
...
def impute_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, graph_dec, decoder1, decoder2, optimizer,
                    RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized,
                    edge_index, emb_size, random_matrix, device, current_epoch, epochs):
    encoder1.eval()
    encoder2.eval()
