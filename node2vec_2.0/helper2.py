...
def generate_feature_matrix(gene_exp_normalized, peak_exp_normalized, emb_size, lookup, random_matrix, device):
    ...
    # org_feature_matrix = lookup.to(device) * concatenated.repeat(1, emb_size).to(device)
    ...
    concatenated = concatenated.repeat(1, emb_size)
    return concatenated
...
def train_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, graph_dec, decoder1, decoder2, optimizer,
    ...
    for cell in range(cell_num):
        ...
        rna_kl.append(kl_theta1)
        atac_kl.append(kl_theta2)
    ...
    # kl_theta1_list = sum(rna_kl)/ len(rna_kl)
    # kl_theta2_list = sum(atac_kl)/ len(atac_kl)
    # print(kl_theta1_list, kl_theta2_list)
    kl_theta1 = -0.5 *(1 + rna_log_sigma - rna_mu.pow(2) - rna_log_sigma.exp()).sum(-1).mean()
    kl_theta2 = -0.5 * (1 + atac_log_sigma - atac_mu.pow(2) - atac_log_sigma.exp()).sum(-1).mean()
    # print(kl_theta1, kl_theta2)
    # +++++++++++++
    recon_loss1 = -(pred_RNA_tensor * RNA_tensor).sum(-1).mean()
    recon_loss2 = -(pred_ATAC_tensor * ATAC_tensor).sum(-1).mean()
...
def babel_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, graph_dec, decoder1, decoder2, optimizer,
                    RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized,
                    edge_index, emb_size, lookup, random_matrix, device, current_epoch, epochs):
    encoder1.train()
    encoder2.train()
    ...
    for cell in range(cell_num):
        ...
        one_cell_recon_ATAC = decoder2(theta1, rho)
        one_cell_recon_RNA = decoder1(theta2, eta)
        rna_impute_list.append(one_cell_recon_ATAC)
        atac_impute_list.append(one_cell_recon_RNA)
    ...
    pred_RNA_tensor = torch.cat(rna_tensor_list, dim=0)
    pred_ATAC_tensor = torch.cat(atac_tensor_list, dim=0)
    impute_RNA_tensor = torch.cat(rna_impute_list, dim=0)
    impute_ATAC_tensor = torch.cat(atac_impute_list, dim=0)
    ...
    recon_loss3 = -(impute_RNA_tensor * RNA_tensor).sum(-1).mean()
    recon_loss4 = -(impute_ATAC_tensor * ATAC_tensor).sum(-1).mean()


    recon_loss = (recon_loss1 + recon_loss2 + recon_loss3 + recon_loss4)
    kl_loss = (kl_theta1 + kl_theta2)
...
def finetune_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, graph_dec, decoder1, decoder2, optimizer,
                    RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized,
                    edge_index, emb_size, lookup, random_matrix, device, current_epoch, epochs):
    encoder1.train()
    encoder2.train()
    ...
    # rna_mse_loss = mse_loss_function(impute_RNA_tensor, RNA_tensor).mean()
    # atac_mse_loss = mse_loss_function(impute_ATAC_tensor, ATAC_tensor).mean()
    ...
    return torch.sum(recon_loss).item(), 0.0

def impute_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, graph_dec, decoder1, decoder2, impute_mod,
                     RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized,
                     edge_index, emb_size, lookup, random_matrix, device, title):
    encoder1.eval()
    encoder2.eval()
    gnn.eval()
    mlp1.eval()
    ...
    with torch.no_grad():
        ...
        mask_rna = torch.zeros_like(RNA_tensor_normalized)
        mask_atac = torch.zeros_like(ATAC_tensor_normalized)
        for cell in range(cell_num):
            if impute_mod == "ATAC":
                one_cell_RNA_tensor_normalized = RNA_tensor_normalized[cell].unsqueeze(0)
                one_cell_ATAC_tensor_normalized = mask_atac[cell].unsqueeze(0)
            else:
                one_cell_RNA_tensor_normalized = mask_rna[cell].unsqueeze(0)
                one_cell_ATAC_tensor_normalized = ATAC_tensor_normalized[cell].unsqueeze(0)
            # one_cell_RNA_tensor = RNA_tensor[cell].unsqueeze(0)
            # sums_RNA = one_cell_RNA_tensor.sum(1).unsqueeze(1)
            # one_cell_ATAC_tensor = ATAC_tensor[cell].unsqueeze(0)
            # sums_ATAC = one_cell_ATAC_tensor.sum(1).unsqueeze(1)
            # print(sums_RNA.shape)
            ...
            eta, rho = split_tensor(emb, ATAC_tensor_normalized.shape[1])
            if impute_mod == "RNA":
                one_cell_pred_RNA_tensor = decoder1(theta2, rho, imputation=True)
                one_cell_pred_ATAC_tensor = decoder2(theta2, eta, imputation=True)
            else:
                one_cell_pred_RNA_tensor = decoder1(theta1, rho, imputation=True)
                one_cell_pred_ATAC_tensor = decoder2(theta1, eta, imputation=True)
            # #
            # one_cell_pred_RNA_tensor = decoder1(theta1)
            # one_cell_pred_ATAC_tensor = decoder2(theta2)
        ...
        # Unnormalize
        sums_ATAC = ATAC_tensor.sum(1).unsqueeze(1)
        pred_ATAC_tensor = pred_ATAC_tensor * (sums_ATAC)
        sums_RNA = RNA_tensor.sum(1).unsqueeze(1)
        pred_RNA_tensor = pred_RNA_tensor * (sums_RNA)

        recon_atac = np.squeeze(pred_ATAC_tensor.reshape([1, -1])).cpu()
        true_atac = np.squeeze(ATAC_tensor.reshape([1, -1])).cpu()
        recon_rna = np.squeeze(pred_RNA_tensor.reshape([1, -1])).cpu()
        true_rna = np.squeeze(RNA_tensor.reshape([1, -1])).cpu()

        if impute_mod == "ATAC":
            correlation_pear = stats.pearsonr(recon_atac, true_atac)[0]
            correlation_spear = stats.spearmanr(recon_atac, true_atac)[0]
            view_results.plot_imputation(recon_atac, true_atac, f"{emb_size}_{title}_node2vec_unnormalized",
                                         './plot_Nov14/', impute_mod,
                                         round(correlation_pear, 4), round(correlation_spear, 4))
        else:
            correlation_pear = stats.pearsonr(recon_rna, true_rna)[0]
            correlation_spear = stats.spearmanr(recon_rna, true_rna)[0]
            view_results.plot_imputation(recon_rna, true_rna, f"{emb_size}_{title}_node2vec_unnormalized",
                                         './plot_Nov14/', impute_mod,
                                         round(correlation_pear, 4), round(correlation_spear, 4))

    return correlation_pear, correlation_spear
