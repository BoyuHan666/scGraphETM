...
def reparameterize(mu, logsig):

    normal = Normal(loc=mu, scale=torch.exp(logsig))
...
def generate_feature_matrix(gene_exp_normalized, peak_exp_normalized, emb_size, lookup, random_matrix, device):
    ...
    onecell_feature_matrix = concatenated.repeat(1, emb_size).to(device)

    feature_matrix = lookup.to(device) * (1 + onecell_feature_matrix)

    return onecell_feature_matrix
...
def train_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, graph_dec, decoder1, decoder2, optimizer,
    ...
    for cell in range(cell_num):
        ...
        mu1, log_sigma1, kl1 = encoder1(one_cell_RNA_tensor_normalized)
        mu2, log_sigma2, kl2 = encoder2(one_cell_ATAC_tensor_normalized)
        ...
        #
        # recon_loss1 = -(one_cell_pred_RNA_tensor * one_cell_RNA_tensor_normalized).sum(-1)
        # print(one_cell_ATAC_tensor)
    ...
    kl_theta1 = torch.tensor(rna_kl).to('cuda:0').mean()
    kl_theta2 = torch.tensor(atac_kl).to('cuda:0').mean()

    recon_loss1 = -(pred_RNA_tensor * RNA_tensor_normalized).sum(-1).mean()
    recon_loss2 = -(pred_ATAC_tensor * ATAC_tensor_normalized).sum(-1).mean()
...
def impute_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, graph_dec, decoder1, decoder2, impute_mod,
    ...
    with torch.no_grad():
        ...
        for cell in range(cell_num):
            ...
            # one_cell_pred_RNA_tensor = decoder1(theta1, rho, imputation=False) + torch.log(sums_RNA)
            # one_cell_pred_ATAC_tensor = decoder2(theta1, eta, imputation=False) + torch.log(sums_ATAC)
            if impute_mod == "RNA":
        ...
        # recon_rna = np.log(1+recon_rna)
        # true_rna = np.log(1+true_rna)
        # recon_atac= np.log(1+recon_atac)
        # true_atac = np.log(1+true_atac)
        if impute_mod == "ATAC":
            correlation_pear = stats.pearsonr(recon_atac, true_atac)[0]
            correlation_spear = stats.spearmanr(recon_atac, true_atac)[0]
            view_results.plot_imputation(recon_atac, true_atac, f"{emb_size}_node2vec_unnormalized", './plot/PBMC/imputation/Jan8/', impute_mod,
            ...
            view_results.plot_imputation(recon_rna, true_rna, f"{emb_size}_node2vec_unnormalized", './plot/PBMC/imputation/Jan8/', impute_mod,
                                         round(correlation_pear, 4), round(correlation_spear, 4))
...
def get_z_GNN(encoder1, encoder2, gnn, mlp1, mlp2, decoder1, decoder2,
                  RNA_tensor_normalized, ATAC_tensor_normalized):
    encoder1.eval()
    encoder2.eval()
    gnn.eval()
