...
def generate_feature_matrix_norm(peak_exp, gene_exp):
    feature_matrix_sparse = vstack([peak_exp.T, gene_exp.T])
    feature_matrix_dense = feature_matrix_sparse.A
    feature_matrix_tensor = torch.from_numpy(feature_matrix_dense)
    return feature_matrix_tensor
...
def process_mini_batch_data(scRNA_adata, scATAC_adata, device,
    ...
    for i in range(batch_num):
        ...
        # feature_matrix = generate_feature_matrix_norm(X_atac_tensor, X_rna_tensor).to(torch.float32)
        # feature_matrix[feature_matrix >= 0.5] = 1.0
        # feature_matrix[feature_matrix < 0.5] = 0.0
