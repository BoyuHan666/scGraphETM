...
def get_gpu_with_most_free_memory():
    ...
    gpu_memory_free = re.findall(r"(\d+) MiB", output)
    gpu_memory_free = [memory for memory in gpu_memory_free]
    print(gpu_memory_free)
    gpu_with_most_free_memory = gpu_memory_free.index(max(gpu_memory_free))
