...
class GNN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_heads, device, dropout, conv_model):
        ...
        if conv_model == 'Transformer':
            ...
            self.conv2 = TransformerConv(hidden_channels * num_heads, out_channels, heads=1, concat=True, dropout=dropout)
        if conv_model == 'GAT':
...
class LDEC(nn.Module):
    def __init__(self, num_modality, emd_size, num_topics):
