...
def train_one_epoch(encoder1, encoder2, gnn, mlp1, mlp2, decoder1, decoder2, optimizer,
    ...
    # recon_loss = (recon_loss1 + recon_loss2 + recon_loss3 + recon_loss4).mean()
    ...
    torch.nn.utils.clip_grad_norm_(encoder1.parameters(), 2.0)
    torch.nn.utils.clip_grad_norm_(encoder2.parameters(), 2.0)
    torch.nn.utils.clip_grad_norm_(gnn.parameters(), 2.0)
    torch.nn.utils.clip_grad_norm_(mlp1.parameters(), 2.0)
    torch.nn.utils.clip_grad_norm_(mlp2.parameters(), 2.0)
    torch.nn.utils.clip_grad_norm_(decoder1.parameters(), 2.0)
    torch.nn.utils.clip_grad_norm_(decoder2.parameters(), 2.0)
...
def process_data(rna_path, atac_path, device, num_of_cell, num_of_gene, num_of_peak, emb_size, cor):
