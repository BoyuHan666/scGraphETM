if __name__ == '__main__':
    ...
    path = f"data/matrix_{str(num_of_cell)}_{str(num_of_gene)}_{str(num_of_peak)}.pth"
    # path1 = "data/matrix_9631_2000_30499.pth"
    # path2 = "data/matrix_2000_2000_8000.pth"
    # path3 = "data/matrix_2000_2000_20000.pth"
    print(f"loading to the file from {path}")
    cor_mat = torch.load(path)
    ...
    class moETMdecoder(nn.Module):
        ...
        def forward(self, theta, beta_Gene=None, beta_Peak=None, cross_prediction=False):
            if beta_Gene is None and beta_Peak is None:
                ...
                # print(theta.shape, beta_Gene.shape, beta_Peak.shape)
                recon_mod1 = torch.mm(theta, beta_Gene)
                recon_mod2 = torch.mm(theta, beta_Peak)
    ...
    class GCN_ETM(nn.Module):
        def __init__(self, num_topics, num_gene, num_peak, t_hidden_size, t_hidden_size_peak, rho_size, eta_size,
                     ...
                     use_gcn=False, feature_matrix=None, edge_index=None, gcn_model=None):
        ...
        def get_theta(self, normalized_bows_gene, normalized_bows_peak):
            ...
            mu, log_sigma = self.pog(Mu, Log_sigma)
            ...
            return theta, theta_d, kld_theta_d, theta_j, kld_theta_j
    ...
    def train_GCNscETM_helper(model, RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized, KL_weight):
        ...
        model.train_gcn()
        nll, kl_theta, nll_mod1, nll_mod2 = model(RNA_tensor, RNA_tensor_normalized, ATAC_tensor,
    ...
    def get_theta_GCN(model, RNA_tensor_normalized, ATAC_tensor_normalized):
        model.eval()
        with torch.no_grad():
            theta_all, theta_d, kld_theta_d, theta_j, kld_theta_j = model.get_theta(RNA_tensor_normalized,
                                                                                    ATAC_tensor_normalized)
    ...
    def train_GCNscETM(model, RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized, adata, adata2,
        ...
        for i in range(niter):
            ...
            NELBO = train_GCNscETM_helper(model, RNA_tensor, RNA_tensor_normalized, ATAC_tensor, ATAC_tensor_normalized,
                                          KL_weight)
            theta_all, theta, theta_gene, theta_peak = get_theta_GCN(model, RNA_tensor_normalized,
                                                                     ATAC_tensor_normalized)
            perf[i, 0] = i
            perf[i, 1] = NELBO
            if (i % ari_freq == 0):
    ...
    model, GNN_ETM_perf, ari_perf, best_ari, theta, theta_all, theta_g, theta_p = train_GCNscETM(model,
                                                                                                 scRNA_1000_tensor,
                                                                                                 scRNA_1000_tensor_normalized,
                                                                                                 scATAC_1000_tensor,
                                                                                                 scATAC_1000_tensor_normalized,
                                                                                                 scRNA_1000_mp_anndata,
                                                                                                 scATAC_1000_mp_anndata,
                                                                                                 ari_freq,
                                                                                                 num_of_epochs)
