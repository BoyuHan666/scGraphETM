...
def cal_cor(scRNA_adata_mini_batch, scATAC_adata_mini_batch, num_of_gene, num_of_peak, cor):
    ...
    cor_mat = torch.zeros(num_of_gene, num_of_peak)
    ...
    for i, gene in enumerate(list(gene_pos_dic.keys())):
        for j, peak in enumerate(list(peak_pos_dic.keys())):
            ...
            if gene_chrom == peak_chrom and (
                    abs(gene_start - peak_start) <= 2000 or (peak_mid > gene_start and peak_mid < gene_end)):
                cor_mat[i, j] = 1
    ...
    return gene_correlation_matrix, edge_index
...
def get_val_data(start, end, num_of_gene, num_of_peak, scRNA_adata, scATAC_adata, cor, feature_matrix, device):
   ...
   """
    ...
    =====================================================================================
    """
    test_gene_correlation_matrix, test_edge_index = cal_cor(scRNA_adata_test,
                                                            scATAC_adata_test,
...
def process_mini_batch_data(rna_path, atac_path, device,
    ...
    for i in range(batch_num):
        ...
        =====================================================================================
        """
        # gene_correlation_matrix, edge_index = cal_cor(scRNA_adata_mini_batch,
