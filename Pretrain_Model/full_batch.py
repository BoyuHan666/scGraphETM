...
def cal_cor(scRNA_adata, scATAC_adata, num_of_gene, num_of_peak, cor):
    ...
    matrix1 = gene_expression.T
    matrix2 = peak_expression.T
    correlation_matrix = np.zeros((matrix1.shape[0], matrix2.shape[0]))

    for i in range(matrix1.shape[0]):
        for j in range(matrix2.shape[0]):
            coef = np.corrcoef(matrix1[i], matrix2[j], rowvar=True)[0][1]
    ...
    return gene_correlation_matrix, peak_correlation_matrix, edge_index, sparse_cor_mat
...
def get_val_data(start, end, num_of_gene, num_of_peak, scRNA_adata, scATAC_adata, cor, feature_matrix, device):
    ...
    =====================================================================================
    """
    test_gene_correlation_matrix, test_peak_correlation_matrix, test_edge_index, test_cor_mat = cal_cor(scRNA_adata_test,
                                                                                          scATAC_adata_test,
    ...
    # test_cor_mat = test_cor_mat.to(device)
    ...
    test_set = (X_rna_test_tensor, X_rna_test_tensor_normalized, X_atac_test_tensor,
                ...
                feature_matrix, test_cor_mat, test_edge_index)
...
def process_full_batch_data(rna_path, atac_path, device,
    ...
    =====================================================================================
    """
    gene_correlation_matrix, peak_correlation_matrix, edge_index, cor_mat = cal_cor(scRNA_adata_full_batch,
    ...
    training_set = (X_rna_tensor, X_rna_tensor_normalized, X_atac_tensor, X_atac_tensor_normalized,
                    ...
                    peak_correlation_matrix, feature_matrix, cor_mat, edge_index, X_rna_tensor_copy, X_atac_tensor_copy)
