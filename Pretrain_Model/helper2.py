...
def evaluate_ari(cell_embed, adata):
    ...
    sc.pp.neighbors(adata, use_rep="cell_embed", n_neighbors=50)
...
def train_one_epoch(encoder1, encoder2, gnn, gnn_decoder, mlp1, mlp2, decoder1, decoder2, optimizer1,
    ...
    if use_mask:  # double side mask
        RNA_tensor_normalized = RNA_tensor_normalized * mask1
        ATAC_tensor_normalized = ATAC_tensor_normalized * mask2
    ...
    torch.nn.utils.clip_grad_norm_(gnn_decoder.parameters(), clamp_num)
...
def get_theta_GNN(encoder1, encoder2, gnn, gnn_decoder, mlp1, mlp2, decoder1, decoder2, RNA_tensor, ATAC_tensor, RNA_tensor_normalized,
...
def get_beta_GNN(encoder1, encoder2, gnn, gnn_decoder,mlp1, mlp2, decoder1, decoder2,
...
def train(encoder1, encoder2, gnn, mlp1, mlp2, decoder1, decoder2,
    ...
    for i in range(niter):
        ...
        theta, theta_gene, theta_peak = get_theta_GNN(encoder1, encoder2, gnn, mlp1, mlp2, decoder1, decoder2, RNA_tensor,
                                                      ATAC_tensor, RNA_tensor_normalized, ATAC_tensor_normalized, metric)
...
def monitor_perf(perf, ari_perf, ari_freq, objective, path, title):
    niter = []
    ari_niter = []
    mse = []
    ari = []
