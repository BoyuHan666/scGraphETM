...
def train(model_tuple, optimizer1, optimizer2, train_set, total_training_set, test_set, metric, ari_freq, niter, use_mlp):
    ...
    print(f"total train set tensor dim: {total_X_rna_tensor_normalized.shape}")
    ...
    for i in range(niter):
        ...
        NELBO = helper2.train_one_epoch_pog_zinb(
            encoder1, encoder2, gnn, mlp1, mlp2, pog_decoder, optimizer1, X_rna_tensor,
        ...
        if i % ari_freq == 0:
            ...
            # beta_gene, beta_peak = helper2.get_beta_GNN_pog(
            #     encoder1, encoder2, gnn, mlp1, mlp2, pog_decoder,
            #     X_rna_test_tensor_normalized, X_atac_test_tensor_normalized,
            #     test_feature_matrix, test_edge_index, use_mlp)

            res, ari, nmi = helper2.evaluate_ari(theta.to('cpu'), scRNA_test_anndata)
            res_train, ari_train, nmi_train = helper2.evaluate_ari(theta_train.to('cpu'), total_scRNA_anndata)
...
if __name__ == "__main__":
    ...
    # pog_decoder = model2.POG_DEC(num_of_gene, num_of_peak, emb_size, num_of_topic, num_of_cell).to(device)
    pog_decoder = model2.POG_DEC_ZINB(num_of_gene, num_of_peak, emb_size, num_of_topic, num_of_cell).to(device)
    ...
    (X_rna_test_tensor, X_rna_test_tensor_normalized, X_atac_test_tensor,
     ...
     test_gene_correlation_matrix, test_peak_correlation_matrix,
     test_feature_matrix, test_edge_index) = test_set
    view_result.generate_clustermap(best_train_theta, scRNA_test_anndata, plot_path_rel)
